...
/**
 * @type {object}
 * @todo Need a better way to do this. Okay for now since factors never change and can there for presist arcoss runs of the same import.
 */
const factorTracker = {};
...
export function computeFactors(n) {
  ...
  for (let test = 2; test < n; test++) {
    if (n % test == 0) {
      ...
      // these will also be factors of n
      // But to avoid endless loop do not retest input n
      if (n != test) {
        let testResults = undefined;
        // For performance sake, look up factors instead of re-computing
        if (factorTracker[test] == undefined) {
          testResults = computeFactors(test); // recursive call
        } else if (factorTracker[test] != undefined) {
          testResults = factorTracker[test];
        }
      ...
      if (n != result) {
        let resultResults = undefined;
        // For performance sake, look up factors instead of re-computing
        if (factorTracker[result] == undefined) {
          resultResults = computeFactors(result); // recursive call
        } else if (factorTracker[result] != undefined) {
          resultResults = factorTracker[result];
        }
        // Merge the data into the return object
